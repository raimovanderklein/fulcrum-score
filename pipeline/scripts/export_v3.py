...
Reads the database and emits markdown with errata, methodology, backtested
...
def main():
    ...
    md = []
    md.append("# Encounter Living Scorecard v3")
    md.append("")
    ...
    for k, v in sorted(hit_breakdown.items()):
        md.append(f"- {k}: {v}")
    md.append("")
    md.append("**Prospective predictions awaiting readout:** 6 (VOLGA, ENERGIZE, IMbrave251, KEYLYNK-012, Harmony Melanoma, DeLLphi-312)")
    md.append("")
    md.append("**Backtested directional hits:** 4 (KeyVibe-006, LATIFY, TiNivo-2, STAR-221) — all FAIL calls on TIGIT/ATR/PARP/IO-continuation classes.")
    md.append("")
    md.append("**Fully prospective hits (so far):** 0. All current hits are backtested.")
    md.append("")
    md.append("---")
    md.append("")
    ...
    md.append("---")
    md.append("")
    md.append("## Backtested directional hits")
    md.append("")
    md.append("These four trials had already publicly read out before v3 was generated. The framework's structural argument applied retrospectively correctly identifies all four as FAILs.")
    md.append("")

    backtested_trials = ['keyvibe_006', 'latify', 'tinivo_2', 'star_221']
    for tid in backtested_trials:
    ...
    md.append("")
    md.append("5. **Backtested predictions are class-level, not formula-derived.** The four backtested hits are scored as `method='backtested'` because they were generated from class-level structural arguments rather than from `formula.predict_orr`. They could be re-derived through the formula if their drug agents were added to the relevant disease domains.")
    md.append("")
    md.append("---")
    md.append("")
    ...
    md.append("python3 seed_backtested.py         # add backtested historical trials and readouts")
